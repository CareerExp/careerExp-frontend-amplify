...
const EducationalInstitutionModal = ({ open, onClose }) => {
  ...
  const addDocument = (type) => {
    if (documents.length >= 3) {
      dispatchToRedux(notify({ type: "warning", message: "Maximum 3 documents allowed" }));
      return;
    }

    const lastDoc = documents[documents.length - 1];
    if (lastDoc && (!lastDoc.name || !lastDoc.value)) {
      dispatchToRedux(notify({ type: "warning", message: "Please fill the current document card first" }));
      return;
    }
                ...
                startIcon={
                  <Box
                    sx={{
                      ...
                      fontSize: "12px",
                      fontWeight: "bold",
                      color: documents.length >= 3 ? "#9e9e9e" : "#bc2876",
                    }}
                  >
                    +
                  </Box>
                  ...
                  {doc.type === "FILE" ? (
                    <Grid item xs={12}>
                      ...
                      >
                        ...
                        >
                          {doc.value ? doc.value.name : "Click to upload file"}
                        </Typography>
                        <input
                          type="file"
                          id={`file-${doc.id}`}
