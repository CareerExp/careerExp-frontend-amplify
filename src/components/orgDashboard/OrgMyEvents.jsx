    ...
    >
      ...
      >
        <Box
          component="img"
          src={
            event.banner || event.coverImage || event.image || eventsPlaceholder
          }
      ...
      <Box sx={{ flexGrow: 1 }}>
          ...
          <Box>
            ...
            >
              ...
              {event.referenceNumber ||
                (event._id ? event._id.slice(-8).toUpperCase() : "N/A")}
            </Typography>
          </Box>
        ...
        >
          ...
          >
            ...
            >
              {String(event.totalCtaClicks || event.ctasReceived || 0).padStart(
                2,
                "0",
              )}
            </Typography>
  ...
  const confirmDelete = async () => {
    ...
    try {
      const resultAction = await dispatch(
        deleteEvent({ id: eventToDelete._id, token }),
      );
      if (deleteEvent.fulfilled.match(resultAction)) {
        ...
        dispatch(
          notify({
            message: resultAction.payload?.error || "Failed to delete event",
            type: "error",
          }),
        );
      }
