...
import { sendInvitation, getOrganizationCounsellors, selectOrganizationCounsellors, selectOrganizationLoading } from '../../redux/slices/organizationSlice';
...
const InvitedCard = ({ counsellor, onClick }) => {
    ...
    const invitedDate = counsellor.invitedAt ? new Date(counsellor.invitedAt).toLocaleDateString() : (counsellor.invitation?.invitedOn || "N/A");
        ...
        >
            ...
            <Box
                sx={{
                    ...
                    background: 'linear-gradient(135deg, rgba(191, 47, 117, 0.1) 0%, rgba(114, 3, 97, 0.1) 100%)',
                    display: 'flex',
            ...
            {/* Content Section */}
            <Box sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column', gap: '10px' }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <Box sx={{ flexGrow: 1 }}>
                        ...
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                            ...
                            >
                                <Typography sx={{ fontFamily: fonts.sans, fontSize: '12px', color: '#ff8a00' }}>
                                    Invited
                                </Typography>
                            </Box>
                            <Box sx={{ width: '5px', height: '5px', borderRadius: '50%', backgroundColor: '#D9D9D9' }} />
                            <Box
                                sx={{
                            ...
                            >
                                ...
                                <Typography sx={{ fontFamily: fonts.sans, fontWeight: 500, fontSize: '11px', color: '#ff8a00' }}>
                                    Pending
                                </Typography>
                            </Box>
                        </Box>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    ...
                    <Typography sx={{ fontFamily: fonts.sans, fontSize: '13px', fontWeight: 500, color: '#bc2876' }}>
                        Invited on: <span style={{ fontWeight: 400, color: 'rgba(0,0,0,0.5)' }}>{invitedDate}</span>
                    </Typography>
                </Box>
...
const AcceptedCard = ({ counsellor, onClick }) => {
    const creator = counsellor.creatorUserId || {};
    const name = creator.firstName ? `${creator.firstName} ${creator.lastName}` : (counsellor.name || "Active Counselor");
    const joinedDate = counsellor.joinedAt ? new Date(counsellor.joinedAt).toLocaleDateString() : (counsellor.joinedOn || "N/A");
        ...
        >
            ...
            >
                {creator.profilePicture ? (
                    <Box component="img" src={creator.profilePicture} sx={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                ) : (
                    <Typography sx={{ color: '#999', fontSize: '12px' }}>No Image</Typography>
                )}
            </Box>

            {/* Content Section */}
            <Box sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column', gap: '10px', overflow: 'hidden' }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <Box sx={{ flexGrow: 1 }}>
                        <Typography
                            sx={{
                        ...
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                            ...
                            {counsellor.isAdmin && (
                                <>
                                    ...
                                    >
                                        <Typography sx={{ fontFamily: fonts.sans, fontSize: '12px', color: '#bc2876' }}>
                                            Admin
                                        </Typography>
                                    </Box>
                                </>
                ...
                <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 1 }}>
                    <LocationOnIcon sx={{ color: 'rgba(0,0,0,0.5)', fontSize: '18px', mt: 0.2 }} />
                    <Typography
                        sx={{
                            fontFamily: fonts.sans,
                            fontSize: '13px',
                    ...
                    >
                        {creator.country || creator.address || counsellor.address || "Location not provided"}
                    </Typography>
                </Box>

                <Box sx={{ display: 'flex', gap: 2 }}>
                    <Typography sx={{ fontFamily: fonts.sans, fontSize: '13px', color: 'rgba(0,0,0,0.5)' }}>
                        <span style={{ fontWeight: 500, color: '#bc2876' }}>Joined :</span> {joinedDate}
                    </Typography>
                </Box>
                ...
                >
                    {[
                        ...
                        <React.Fragment key={stat.label}>
                            ...
                            {idx < 3 && (
                                <Divider orientation="vertical" flexItem sx={{ borderColor: 'rgba(0,0,0,0.1)', height: '40px', alignSelf: 'center' }} />
                            )}
                        </React.Fragment>
                    ))}
                </Box>
...
const OrgMyCounsellors = () => {
    ...
    const handleInviteCounsellor = async (formData) => {
        if (!token) {
            dispatch(notify({ message: "You must be logged in to send invitations", type: "error" }));
            return;
        }

        try {
            const resultAction = await dispatch(sendInvitation({ inviteData: formData, token }));
            if (sendInvitation.fulfilled.match(resultAction)) {
                dispatch(notify({ message: "Invitation sent successfully!", type: "success" }));
                // Refresh the list to show the newly invited counselor
                dispatch(getOrganizationCounsellors({ token }));
                handleCloseInviteModal();
            } else {
                const errorMsg = resultAction.payload?.error || "Failed to send invitation";
            ...
            {isLoading ? (
                ...
                <Grid container spacing={2.25}>
                    {counsellors.map((counsellor) => (
                        <Grid item key={counsellor._id || counsellor.id} xs={12} lg={6} sx={{ display: 'flex', justifyContent: 'center' }}>
                            {(counsellor.status === 'INVITED' || counsellor.inviteStatus === 'sent') ? (
                                <InvitedCard
                ...
                <Box sx={{ textAlign: 'center', py: 8 }}>
                    <Typography sx={{ fontFamily: fonts.sans, fontSize: '18px', color: '#787876' }}>
