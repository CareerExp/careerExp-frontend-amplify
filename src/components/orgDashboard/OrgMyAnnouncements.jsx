        ...
        >
            ...
            <Box sx={{ flexGrow: 1 }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 1 }}>
                    <Box>
                        ...
                        >
                            Ref no – {announcement.refNo || (announcement._id ? announcement._id.slice(-8).toUpperCase() : 'N/A')}
                        </Typography>
                    </Box>
                ...
                <Box sx={{ display: 'flex', gap: 2.5, mb: 2 }}>
                    <Typography sx={{ fontFamily: fonts.sans, fontSize: '13px', color: '#BC2876', fontWeight: 500 }}>
                        Live Start : <span style={{ color: 'rgba(0,0,0,0.5)', fontWeight: 400 }}>{formatDate(announcement.liveStartDate || announcement.startDate)}</span>
                    </Typography>
                    <Typography sx={{ fontFamily: fonts.sans, fontSize: '13px', color: '#BC2876', fontWeight: 500 }}>
                        Live End : <span style={{ color: 'rgba(0,0,0,0.5)', fontWeight: 400 }}>{formatDate(announcement.liveEndDate || announcement.endDate)}</span>
                    </Typography>
                </Box>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
                    ...
                    >
                        ...
                        >
                            {String(announcement.totalCtaClicks || announcement.ctasReceived || 0).padStart(2, '0')}
                        </Typography>
                    </Box>
                </Box>
            </Box>
            ...
            >
                ...
                <MenuItem onClick={handleClose} sx={{ gap: 1.5, py: 1.2 }}>
                    ...
                    <Typography sx={{ fontFamily: fonts.sans, fontSize: '14|px', color: '#344054' }}>Archive</Typography>
                </MenuItem>
                <Divider sx={{ my: '0 !important' }} />
...
const OrgMyAnnouncements = () => {
    ...
    const confirmDelete = async () => {
        ...
        setIsDeleting(true);
        try {
            const resultAction = await dispatch(deleteAnnouncement({ id: announcementToDelete._id, token }));
            if (deleteAnnouncement.fulfilled.match(resultAction)) {
                dispatch(notify({ message: "Announcement deleted successfully!", type: "success" }));
                setIsDeleteModalOpen(false);
                setAnnouncementToDelete(null);
            } else {
                dispatch(notify({ message: resultAction.payload?.error || "Failed to delete announcement", type: "error" }));
            }
        } catch (error) {
