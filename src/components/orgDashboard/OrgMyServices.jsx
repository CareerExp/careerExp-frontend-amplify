...
import { fetchMyServices, selectMyServices, selectServiceLoading, deleteService } from '../../redux/slices/serviceSlice';
            ...
            <Box sx={{ flexGrow: 1 }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 1 }}>
                    <Box>
                        ...
                        >
                            Ref no – {service.referenceNumber || (service._id ? service._id.slice(-8).toUpperCase() : 'N/A')}
                        </Typography>
                    </Box>
                ...
                <Box sx={{ display: 'flex', gap: 2.5, mb: 2 }}>
                    <Typography sx={{ fontFamily: fonts.sans, fontSize: '13px', color: '#BC2876', fontWeight: 500 }}>
                        Price : <span style={{ color: 'rgba(0,0,0,0.5)', fontWeight: 400 }}>
                            {service.priceType === 'FREE' ? 'Free' : service.priceType === 'CUSTOM' ? 'Custom' : `${service.currency || 'INR'} ${service.price}`}
                        </span>
                    </Typography>
                    <Typography sx={{ fontFamily: fonts.sans, fontSize: '13px', color: '#BC2876', fontWeight: 500 }}>
                        Duration : <span style={{ color: 'rgba(0,0,0,0.5)', fontWeight: 400 }}>
                            {service.duration ? `${service.duration.value} ${service.duration.unit}` : 'N/A'}
                        </span>
                    </Typography>
                </Box>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
                    ...
                    >
                        ...
                        >
                            {String(service.totalCtaClicks || 0).padStart(2, '0')}
                        </Typography>
                    </Box>
                </Box>
            </Box>
...
const OrgMyServices = () => {
    ...
    const confirmDelete = async () => {
        ...
        setIsDeleting(true);
        try {
            const resultAction = await dispatch(deleteService({ id: serviceToDelete._id, token }));
            if (deleteService.fulfilled.match(resultAction)) {
                dispatch(notify({ message: "Service deleted successfully!", type: "success" }));
                setIsDeleteModalOpen(false);
                setServiceToDelete(null);
            } else {
                dispatch(notify({ message: resultAction.payload?.error || "Failed to delete service", type: "error" }));
            }
        } catch (error) {
                            ...
                            {services.map((service) => (
                                <Grid item key={service._id} xs={12} sm={6} lg={4}>
                                    <ServiceCard
                                        ...
                                        onView={(srv) => setSelectedService(srv)}
                                    />
                                </Grid>
                            ))}
                        </Grid>
