...
const AddAnnouncement = ({ onBack, announcementToEdit }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    liveStartDate: announcementToEdit?.liveStartDate
      ? new Date(announcementToEdit.liveStartDate).toISOString().split("T")[0]
      : "",
    liveEndDate: announcementToEdit?.liveEndDate
      ? new Date(announcementToEdit.liveEndDate).toISOString().split("T")[0]
      : "",
  ...
  const [imagePreview, setImagePreview] = useState(
    announcementToEdit?.coverImage || null,
  );
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      !formData.liveStartDate ||
      !formData.liveEndDate ||
      !formData.ctaValue
    ) {
      dispatch(
    ...
    try {
      ...
      data.append(
        "liveStartDate",
        new Date(formData.liveStartDate).toISOString(),
      );
      ...
      if (announcementToEdit) {
        ...
        if (updateAnnouncement.fulfilled.match(updateAction)) {
          ...
          dispatch(
            notify({
              message:
                updateAction.payload?.error || "Failed to update announcement",
              type: "error",
            }),
          );
        }
        ...
        const createAction = await dispatch(
          createAnnouncement({ formData: data, token }),
        );
        if (createAnnouncement.fulfilled.match(createAction)) {
          ...
          dispatch(
            notify({
              message:
                createAction.payload?.error || "Failed to create announcement",
              type: "error",
            }),
          );
        }
          ...
          <Grid container spacing={3.5}>
            ...
            <Grid item xs={12}>
              ...
              <TextField
                ...
                helperText={`${(formData.title || "").length}/160`}
                variant="outlined"
                sx={{
            ...
            <Grid item xs={12}>
              ...
              <TextField
                ...
                helperText={`${(formData.description || "").length}/4000`}
                sx={{
                  "& .MuiOutlinedInput-root": {
                    borderRadius: "24px",
                    backgroundColor: "#f2f2f2",
            ...
            <Grid item xs={12} sm={6}>
              ...
              <TextField
                ...
                onChange={handleChange}
                inputProps={{
                  min: formData.liveStartDate || today,
                }}
                InputLabelProps={{ shrink: true }}
                sx={{
                  "& .MuiOutlinedInput-root": {
