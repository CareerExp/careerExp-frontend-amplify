...
const EventCard = ({ event: ev }) => {
  ...
  const description = ev?.description
    ? stripHtml(ev.description).slice(0, 80) + (stripHtml(ev.description).length > 80 ? "..." : "")
    : "";
  ...
  const dateStr = ev?.eventDate || ev?.startDate || ev?.createdAt || ev?.publishedAt || "";
  const id = ev?._id;

  const rawType = (ev?.eventType || ev?.type || "").toString().toLowerCase().replace(/\s+/g, "_");
  const typeStyle = EVENT_TYPE_STYLES[rawType] || EVENT_TYPE_STYLES[ev?.mode] || { bg: "#f5f5f5", textColor: "#717171", label: ev?.eventType || ev?.type || "Event" };
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <CalendarTodayIcon sx={{ color: FIGMA_ACCENT, fontSize: "1.1rem" }} />
          {formatDate(dateStr)}
        </span>
