...
/** Organization type label for invitation cards */
function getOrgTypeLabel(invite) {
  const orgType =
    invite?.organizationType || invite?.organizationDetails?.organizationType;
  if (orgType === "ESP") return "Education Service Provider";
  if (orgType === "HEI") return "Higher Education Institution";
  return "Higher Education Institution"; // fallback
}
...
const MyCompany = () => {
  ...
  const myOrganization = useSelector(selectMyOrganization);
  ...
  // organizationUserId can come from profile, from accept-flow (myOrganization), or from invitations/me (currentMembership)
  const organizationUserId =
    userData?.organization?.organizationId ||
    userData?.organization?._id ||
    userData?.organization?.id ||
    myOrganization?.organizationUserId ||
    myOrganization?._id ||
    currentMembership?.organizationUserId ||
  ...
  // When member but organizationUserId not in profile, fetch current membership (for Leave button)
  useEffect(() => {
    if (isMember && token && !organizationUserId) {
      dispatch(getCreatorCurrentMembership({ token }));
    }
  }, [dispatch, isMember, token, organizationUserId]);
    ...
    try {
      ...
      if (respondToInvitation.fulfilled.match(resultAction)) {
        ...
        dispatch(
          notify({
            message:
              resultAction.payload?.error || `Failed to ${status} invitation`,
            type: "error",
          }),
        );
      }
  ...
  const handleLeaveCompany = async () => {
    if (!organizationUserId || !token) {
      dispatch(
        notify({
          message: "Unable to leave: missing membership data.",
          type: "error",
        }),
      );
      return;
    ...
    if (
      !window.confirm(
        "Are you sure you want to leave this company? You can rejoin if invited again.",
      )
    ) {
      return;
    }
    ...
    try {
      const resultAction = await dispatch(
        leaveOrganization({ organizationUserId, token }),
      );
      if (leaveOrganization.fulfilled.match(resultAction)) {
        dispatch(
          notify({
        ...
        dispatch(
          notify({
            message:
              resultAction.payload?.error || "Failed to leave organization",
            type: "error",
          }),
        );
      }
      ...
      <Box sx={{ p: 4, backgroundColor: "#f9fafb", minHeight: "100%" }}>
        ...
        >
          ...
          <Button
            ...
            disabled={isLeaving || !organizationUserId}
            sx={{
              borderColor: "#D92D20",
              color: "#D92D20",
              borderRadius: "90px",
      ...
      {isLoading && invitations.length === 0 ? (
        ...
        <>
          ...
          >
            ...
            >
              By accepting this invitation, your profile and content may appear
              on the company’s public profile, based on their visibility
              settings. You can leave the company at any time from this page.
            </Typography>
          </Paper>
